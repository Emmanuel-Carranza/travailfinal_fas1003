...
![image](img/graph2.png) Les mots de passe de 4 caractères prennent moins de 10 minute à déchiffrers alors que ceux de 9 et plus peuvent prendre des années jusqu'à des décénnies.
...
![image](img/graph3.png) Bien que la banque de données nous indique que la force des mots de passe se limite de 1 a 10 (1 étant le plus faible). On y apercoit 15 au dessus de 10 et 30 mot de passe à 0.
...
dat <- dat %>%
  mutate(type = case_when(
    grepl("[A-Za-z]+", password) & grepl("[0-9]+", password) ~ "Chiffres & lettres",
...
graph3 <- dat %>%
          ...
          labs(x = "Nombre de caractère",
               y = "",
          title = "La longueur vs temps de décryption(sec)")+
          ggthemes::theme_stata() +
          theme(plot.title = element_text(size = 20L, face = "bold.italic"),
                axis.text.y = element_text(angle = 0,vjust = 1, hjust=1),
